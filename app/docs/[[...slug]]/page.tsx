...
import { getDocBySlug, getFirstDoc, docsData } from "@/lib/docs";
...
export async function generateStaticParams() {
  ...
  return getAllDocItems().map((item) => ({
    slug: [item.slug],
  }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const doc = slugStr ? getDocBySlug(slugStr) : getFirstDoc();
  ...
  html = html.replace(
    ...
    (match) => {
      const items = match.trim().split("\n").map((line) => {
        ...
        return `<li class="text-zinc-600 dark:text-zinc-400 leading-relaxed">${text}</li>`;
      });
      return `<ul class="list-disc pl-6 my-4 space-y-1.5">${items.join("")}</ul>`;
    }
  );
  html = html.replace(
    /(^\d+\. .+$\n?)+/gm,
    (match) => {
      ...
      return `<ol class="list-decimal pl-6 my-4 space-y-1.5">${items.join("")}</ol>`;
    }
  );
    ...
    <article className="max-w-none">
      ...
      <div className="mt-16 pt-6 border-t border-zinc-200 dark:border-zinc-800 flex flex-col sm:flex-row gap-3 justify-between">
        {prev ? (
          ...
          >
            <span className="text-xs text-zinc-400 dark:text-zinc-500">Previous</span>
        ...
        {next && (
          ...
          >
            <span className="text-xs text-zinc-400 dark:text-zinc-500">Next</span>
            <span className="text-sm font-medium text-zinc-700 dark:text-zinc-300 group-hover:text-zinc-900 dark:group-hover:text-zinc-100 flex items-center gap-1.5">
              {next.title}
              <ArrowRight size={13} />
            </span>
